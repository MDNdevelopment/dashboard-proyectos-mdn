...
export default function PanoramaView({ teams, tasks, monthIdx, onSelectTeam, onNavigateToBase }) {
  ...
  const ranked = [...stats].sort(
    (a, b) => (b.total ? b.pct : -1) - (a.total ? a.pct : -1) || b.cerradas - a.cerradas,
  )
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <KpiCard
          label="Cumplimiento"
          value={`${gPct}%`}
          sub={gPct >= 90 ? 'Agencia al día' : gPct < 70 && gTotal ? 'Necesita foco' : 'En marcha'}
          color={gPct >= 90 ? '#16A34A' : gPct < 70 && gTotal ? '#E14848' : '#F0871F'}
          onClick={() => onNavigateToBase(null)}
        />
        <KpiCard
          label="Paralizados"
      ...
      {teams.length === 0 ? (
        <div className="bg-white rounded-xl border border-[#e0ddd4] p-10 text-center">
          ...
          <p className="text-[15px] text-[#bbb]">
            Crea el primer team con el botón &ldquo;Gestionar teams&rdquo;
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5 mb-3">
                  ...
                  <div>
                    <p className="text-[15px] font-bold text-[#111] leading-tight">{s.team.name}</p>
