...
function SemLight({ light }) {
  return (
    <span className="inline-flex items-center gap-1.5 text-[14px] font-semibold" style={{ color: light.color }}>
...
export default function PanoramaView({ teams, tasks, monthIdx, onSelectTeam }) {
  const stats = teams.map((t, i) => ({ ...teamMonthStats(t.id, tasks, monthIdx), team: t, palette: teamPalette(i) }))
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        <KpiCard label="Tareas del mes" value={gTotal} sub={fmtMonth(monthIdx)} />
      ...
      {teams.length === 0 ? (
        <div className="bg-white rounded-xl border border-[#e0ddd4] p-10 text-center">
          <p className="text-[16px] font-medium text-[#888] mb-1">Aún no hay teams</p>
          <p className="text-[15px] text-[#bbb]">Crea el primer team con el botón "Gestionar teams"</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-2.5 mb-3">
                  ...
                  <div>
                    <p className="text-[15px] font-bold text-[#111] leading-tight">Team {s.team.name}</p>
                    <p className="text-[13px] text-[#888]">{isLead ? '⭐ Líder del mes' : 'Línea operativa'}</p>
                  </div>
                  <span className="ml-auto text-[13px] font-mono text-[#888]">#{i + 1}</span>
                </div>
                <div className="mb-2">
                  <span className="text-[30px] font-bold leading-none" style={{ color: lg.color }}>
                    {s.total ? `${s.pct}%` : '—'}
                  </span>
      ...
      {mentorNote && (
        <div className="bg-[#fffbeb] border border-[#fde68a] rounded-xl px-4 py-3 flex gap-3 items-start">
          ...
          <p className="text-[15px] text-[#7c6e00]"><b>Dupla de mentoría sugerida:</b> {mentorNote}</p>
        </div>
      )}
    </div>
  )
