            ...
            {ESTADOS.map((s) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span
                    style={{ color: active ? '#111' : '#555', fontWeight: active ? '700' : '500' }}
                  >
                    {s}
                  </span>
...
function DirectionFilter({ directionUsers, selectedIds, onChange }) {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div
          data-testid="direction-filter-panel"
          className="absolute z-50 top-full mt-1 left-0 right-0 bg-white border border-[#e0ddd4] rounded-xl shadow-lg overflow-hidden min-w-[180px]"
        >
                ...
                {directionUsers.map((u) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="flex-1 text-[14px] text-[#333]">
...
export default function BaseView({
  ...
  const [fAlert, setFAlert] = useState(
    () => initialFilter?.alert ?? searchParams.get('fAlert') ?? '',
  ) // '' | 'late' | 'drag' | 'cont' | 'ok'
    ...
    return (
      <div className="bg-white rounded-xl border border-[#e0ddd4] p-10 text-center">
        <p className="text-[16px] font-medium text-[#888]">
          Selecciona un team para ver su base de tareas
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        <div
          className={`grid grid-cols-2 sm:grid-cols-3 gap-2 ${allLines ? 'lg:grid-cols-7' : 'lg:grid-cols-6'}`}
        >
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[14px] text-[#888]">
              {hasFilters ? `${filtered.length} de ${baseTasks.length}` : baseTasks.length} tarea
              {baseTasks.length !== 1 ? 's' : ''}
            </span>
            {activeClientName && (
      ...
      {baseTasks.length === 0 ? (
        <div className="bg-white rounded-xl border border-[#e0ddd4] p-10 text-center">
          ...
          <p className="text-[14px] text-[#bbb]">
            Crea la primera tarea con el botón &ldquo;Nueva tarea&rdquo;
          </p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-[#e0ddd4] p-8 text-center">
          ...
          <button
            onClick={clearFilters}
            className="mt-2 text-[14px] font-semibold text-[#FFB800] hover:underline"
          >
            Limpiar filtros
          </button>
        </div>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-[#333] max-w-[220px]">
                        ...
                        {late ? (
                          ...
                          isContinuous(t) && (
                            <span className="block text-[13px] text-[#F0871F] mt-0.5">
                              Tarea continua
                            </span>
                          )
                        )}
                      ...
                      <td className="px-4 py-3 text-[14px] font-mono text-[#666]">
                        {fmtShort(t.request_date)}
                      </td>
                      <td className="px-4 py-3 text-[14px] font-mono text-[#666]">
                        {late ? (
                          <span className="text-red-500">{fmtShort(t.due_date)}</span>
                        ) : (
                          fmtShort(t.due_date)
                        )}
                      </td>
