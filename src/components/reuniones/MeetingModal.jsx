...
export default function MeetingModal({
  ...
  // Tolera reuniones viejas (solo client_id escalar, sin client_ids) para que al editarlas
  // el picker las muestre preseleccionadas.
  const [clientIds, setClientIds] = useState(
    meeting?.client_ids?.length
      ? meeting.client_ids
      : meeting?.client_id
        ? [meeting.client_id]
        : [],
  ...
  const [startsAt, setStartsAt] = useState(() =>
    toLocalInputValue(meeting?.starts_at ?? defaultDate),
  )
  ...
  async function handleSubmit(e) {
    ...
    if (
      ...
      new Date(newStartsAt).getTime() !== new Date(meeting.starts_at).getTime()
    ) {
          ...
          <div>
            <label className={labelClass}>
              ...
              <span className="normal-case font-normal text-[#bbb]">
                (opcional, si aplica más de una marca)
              </span>
            </label>
            <ClientPicker clients={clients} selectedIds={clientIds} onChange={setClientIds} />
          </div>
