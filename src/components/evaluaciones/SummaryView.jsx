...
export default function SummaryView({ companyId }) {
  ...
  // Convertimos a YYYY-MM-01 para la consulta
        ...
        <div className="bg-white rounded-xl border border-[#e0ddd4] overflow-hidden">
          ...
          <div className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-x-4 items-center">
            ...
            {rows.map((row, idx) => {
              ...
              return [
                /* Nombre */
                <div key={`n${idx}`} className={`flex items-center gap-2.5 min-w-0 ${cellBase}`}>
                ...
                /* Cargo */
                <span key={`c${idx}`} className={`text-[14px] text-[#666] whitespace-nowrap ${cellBase}`}>
                  {row.position_name ?? "—"}
                </span>,
                /* Recibidas */
                <span key={`r${idx}`} className={`text-[15px] font-mono text-[#888] text-right tabular-nums ${cellBase}`}>
                  {row.received_count}
                </span>,
                /* Hechas */
                <span key={`h${idx}`} className={`text-[15px] font-mono text-[#888] text-right tabular-nums ${cellBase}`}>
                  {row.made_count}
                </span>,
                /* Score */
