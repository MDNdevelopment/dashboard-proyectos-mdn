    ...
    <div className="bg-white rounded-2xl border border-[#e0ddd4] px-5 py-4">
      <div className="flex items-center justify-between mb-4">
        ...
        {analysis && !loading && (
          <button
            ...
            className="px-3 py-1.5 rounded-xl text-[14px] font-semibold text-[#555] border border-[#e0ddd4] hover:bg-[#f5f3eb] transition-colors"
          >
            Regenerar
          </button>
        )}
      ...
      {loading && (
        <div className="flex items-center gap-3 py-6">
          ...
          <span className="text-[15px] text-[#888]">Generando análisis…</span>
        </div>
      )}
      ...
      {analysis && !loading && (
        <div className="space-y-5">
          {/* Resumen */}
          <div>
            <p className="text-[13px] font-mono font-bold tracking-[0.1em] uppercase text-[#bbb] mb-1.5">
              Resumen
            </p>
            <p className="text-[15px] text-[#333] leading-relaxed">{analysis.summary}</p>
          </div>
            ...
            return (
              <div key={key}>
                ...
                <ul className="space-y-1.5">
                  {items.map((item, i) => (
                    <li key={i} className="flex items-start gap-2 text-[15px] text-[#333] leading-relaxed">
      ...
      {!analysis && !loading && !error && (
        <p className="text-[15px] text-[#aaa] pb-2">
