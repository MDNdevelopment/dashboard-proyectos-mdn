  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-2">
        <p className="text-[16px] font-semibold text-[#111] leading-snug flex-1">{ticket.title}</p>
        <div className="flex items-center gap-1">
          <span className="text-[12px] font-mono text-[#888] uppercase tracking-wider">Estado:</span>
          <TicketStatusBadge type="status" value={ticket.status} />
        </div>
      </div>
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[12px] font-mono text-[#888] uppercase tracking-wider">Prioridad:</span>
        <TicketStatusBadge type="priority" value={ticket.priority} />
        <span className="text-[12px] font-mono text-[#888] uppercase tracking-wider">Categoría:</span>
        ...
        {sla && (
          <span className={`text-[12px] font-mono font-bold px-2 py-0.5 rounded-md ${sla.color}`}>
            {sla.label}
          </span>
        )}
        {isIT && (
          <span className="text-[13px] text-[#888]">{requesterName}</span>
        )}
        <span className="text-[13px] text-[#aaa] ml-auto">{date}</span>
      </div>
    </button>
  )
}
