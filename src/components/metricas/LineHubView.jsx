...
export default function LineHubView({ line, companyId, year = CURRENT_YEAR }) {
  ...
  // Mes actual, solo si el año mostrado es el año en curso.
  const lastMonthData = year === CURRENT_YEAR ? (monthScores[CURRENT_MONTH - 1] ?? null) : null
  const lastScore = lastMonthData ? sumScore(lastMonthData) : null
  const lastMonth = year === CURRENT_YEAR ? CURRENT_MONTH : null
  ...
  const barData = monthScores.filter(Boolean).map((m) => {
    const row = { mes: MONTHS[m.month - 1].slice(0, 3) }
    INDICATORS.forEach((ind) => {
      row[ind.short] = Number(m[ind.key].toFixed(2))
    })
    return row
  })
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="bg-white rounded-2xl border border-[#e0ddd4] px-5 py-4 flex flex-col items-center justify-center">
          {lastMonthData ? (
            ...
            <p className="text-[14px] text-[#aaa] text-center py-8">
              {lastMonth
                ? `Aún no hay datos de ${MONTHS[lastMonth - 1]} ${year}.`
                : `Sin datos para ${year}.`}
            </p>
          )}
        </div>
          ...
          {lastMonthData ? (
            <div className="space-y-1">
              {INDICATORS.map((ind, i) => {
                const pts = Number(lastMonthData[ind.key].toFixed(1))
                const pct = (pts / ind.peso) * 100
                return (
                  <div key={ind.key} className="flex items-center gap-2">
                    <span className="text-[12px] font-mono text-[#888] w-[90px] flex-shrink-0">
                      {ind.short}
                    </span>
