...
// Siempre disponible el mes siguiente al actual, además del actual, para poder
// adelantar carga de datos (p. ej. campañas/reuniones ya agendadas del próximo mes).
const NEXT_MONTH = Math.min(CURRENT_MONTH + 1, 12)
...
export default function LineView({ line, companyId, onLinesChange }) {
  ...
  // Clamp month: nunca permitir más allá del mes siguiente al actual, en el año actual
  const clampedMonth = rawMonth >= 1 && rawMonth <= 12 ? rawMonth : CURRENT_MONTH
  const month = year === CURRENT_YEAR && clampedMonth > NEXT_MONTH ? NEXT_MONTH : clampedMonth
  ...
  return (
    <div className="space-y-5">
      ...
      {subView === 'hub' && <LineHubView line={line} companyId={companyId} year={year} />}
      {subView === 'operaciones' && (
        <OperacionesView
          line={line}
          companyId={companyId}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl border border-[#e0ddd4] p-6 max-w-sm w-full space-y-4">
            ...
            <p className="text-[13px] text-[#888]">
              Esta acción es permanente: ni Operaciones ni Finanzas de este mes podrán volver a
              editarse, y no hay forma de reabrirlo desde la app.
            </p>
            {closeError && (
