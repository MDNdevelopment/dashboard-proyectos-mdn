...
function ChipSelect({ options, selected, onChange, placeholder, wrapperClass = '' }) {
  ...
  return (
    <div className={`relative ${wrapperClass}`}>
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full min-w-[160px] bg-white border border-[#d4d0c8] rounded-xl shadow-lg overflow-y-auto max-h-56">
          {options.length === 0 && (
          ...
          {options.map((opt) => (
            ...
            >
              <span
                className={`w-4 h-4 rounded flex-shrink-0 border flex items-center justify-center ${selected.includes(opt.value) ? 'bg-[#111] border-[#111]' : 'border-[#ccc]'}`}
              >
          ...
          <div key={i}>
            {i > 0 && (
              <div className="flex items-center gap-2 my-1.5">
                ...
                <span className="text-[11px] font-mono font-bold text-[#bbb] uppercase tracking-widest">
                  Y
                </span>
                <div className="h-px flex-1 bg-[#e0ddd4]" />
              </div>
    ...
    <div className="px-3 py-3 sm:px-5 sm:py-4 border-b border-[#f0ede3] last:border-b-0">
      ...
      <div className="flex flex-col gap-2 mb-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 flex-wrap min-w-0">
          ...
          <span className="hidden sm:inline text-[12px] font-mono text-[#ccc] truncate">
            {capKey}
          </span>
        </div>
          ...
          <div key={i}>
            {i > 0 && (
              <div className="flex items-center gap-3 my-3">
                <div className="h-px flex-1 bg-[#e0ddd4]" />
                <span className="px-3 py-1 bg-[#f5f3eb] text-[12px] font-mono font-bold text-[#888] uppercase tracking-widest rounded-full">
                  O
                </span>
                <div className="h-px flex-1 bg-[#e0ddd4]" />
              </div>
...
function ModuleCard({
  ...
  return (
    <div className="bg-white border border-[#e0ddd4] rounded-xl">
      {/* Header del módulo — clickeable para expandir/colapsar. Antes el contenedor tenía
          overflow-hidden (para recortar las esquinas cuadradas de este header contra el
          rounded-xl del padre), pero eso también recortaba el dropdown de ChipSelect cuando
          se abría cerca del borde inferior de la card — se reemplaza por redondear el propio
          header. */}
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className={`w-full px-3 py-3 sm:px-5 sm:py-3.5 bg-[#fafaf7] flex items-center gap-3 text-left hover:bg-[#f5f3eb] transition-colors rounded-t-xl ${open ? '' : 'rounded-b-xl'}`}
      >
...
export default function PermisosView({ companyId }) {
  ...
  return (
    <div className="space-y-4 relative">
      ...
      {toast && (
        <div
          className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2.5 px-4 sm:px-5 py-3 rounded-xl shadow-lg text-[13px] sm:text-[14px] font-semibold transition-all max-w-[90vw] text-center ${
      ...
      <div className="bg-[#fffbeb] border border-[#ffe58f] rounded-xl px-3 py-3 sm:px-5 sm:py-4 flex gap-3">
        ...
        <div>
          <p className="text-[14px] font-semibold text-[#8a6300] mb-1">
          ...
          <p className="text-[13px] sm:text-[13.5px] text-[#a07800] leading-relaxed">
            ...
            eliminar). Por capacidad podés crear <strong>grupos de condiciones</strong>: el usuario
            accede si cumple <strong>cualquier grupo completo</strong> (lógica <strong>O</strong>).
            Dentro de cada grupo, todas las condiciones deben cumplirse (lógica <strong>Y</strong>).
            Sin reglas = acceso libre. Usá el bloque de <strong>Exclusiones</strong> para bloquear a
            departamentos, usuarios o cargos específicos (ej. «todo el mundo menos Diseño»): si
            alguien cumple <strong>cualquier</strong> exclusión, no puede acceder aunque las reglas
            lo permitan. Los administradores siempre tienen acceso, sin excepción.
          </p>
        </div>
      </div>
