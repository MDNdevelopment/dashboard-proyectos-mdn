...
function BrandLogos({ clients, color }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      {overflow > 0 && <span className="text-[11px] font-mono text-[#aaa] ml-1">+{overflow}</span>}
    </div>
  )
}
...
export default function LinesView({ companyId, canManage = true }) {
  ...
  async function handleSetLeader(lineId, userId) {
    ...
    setLines((prev) => prev.map((l) => (l.id === lineId ? { ...l, lead_user_id: userId } : l)))
  }
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <p className="text-[15px] text-[#888] max-w-xl">
          Gestiona las líneas operativas y asigna empleados a cada una. Al asignar un empleado a una
          línea, se mueve automáticamente si ya pertenecía a otra.
        </p>
        {canManage && (
          <button
        ...
        (() => {
          ...
          return freshLine ? (
            <LineFichaModal
              ...
              onRemoveMember={
                canManage ? (userId) => handleRemoveMember(freshLine.id, userId) : undefined
              }
              onSetLeader={
                canManage ? (userId) => handleSetLeader(freshLine.id, userId) : undefined
              }
              onRemoveLeader={
                canManage ? (userId) => handleRemoveLeader(freshLine.id, userId) : undefined
              }
