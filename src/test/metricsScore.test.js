...
describe('calcCrecimiento', () => {
  ...
  it('retorna 20 cuando el cliente cumple con seguidoresGanados explícito', () => {
    const curr = makeReport({
      crecimiento: {
        items: [{ clienteId, seguidoresGanados: 150, seguidoresActuales: 1150, meta: 100 }],
      },
    })
  ...
  it('retorna 20 cuando todos los clientes cumplen (fallback via prevReport)', () => {
    ...
    const curr = makeReport({
      crecimiento: {
        items: [{ clienteId, seguidoresGanados: null, seguidoresActuales: 1100, meta: 100 }],
      },
    })
...
describe('calcSolicitudes', () => {
  ...
  // ── Desglose CNP + Gestión de Tareas (SOLICITUDES_MODULE_START) ──────────────
  it('con desglose completo reparte 5+5 según cada fuente', () => {
    const r = makeReport({
      solicitudes: {
        solicitudes: 16,
        editadas: 14,
        cnp: { solicitudes: 8, entregados: 8 }, // 5 * 8/8 = 5
        tareas: { solicitudes: 8, entregados: 6 }, // 5 * 6/8 = 3.75
      },
    })
    expect(calcSolicitudes(r)).toBeCloseTo(8.75)
  })

  it('bucket CNP vacío redistribuye sus 5 pts a Tareas', () => {
    const r = makeReport({
      solicitudes: {
        solicitudes: 10,
        editadas: 10,
        cnp: { solicitudes: 0, entregados: 0 },
        tareas: { solicitudes: 10, entregados: 10 },
      },
    })
    expect(calcSolicitudes(r)).toBe(10) // 10 * 10/10, no se pierden los 5 de CNP
  })

  it('bucket Tareas vacío redistribuye sus 5 pts a CNP', () => {
    const r = makeReport({
      solicitudes: {
        solicitudes: 4,
        editadas: 2,
        cnp: { solicitudes: 4, entregados: 2 },
        tareas: { solicitudes: 0, entregados: 0 },
      },
    })
    expect(calcSolicitudes(r)).toBe(5) // 10 * 2/4
  })

  it('ambos buckets vacíos da 0', () => {
    const r = makeReport({
      solicitudes: {
        solicitudes: 0,
        editadas: 0,
  ...
  it('sin desglose (reportes históricos) mantiene la fórmula original — regresión', () => {
    const r = makeReport({ solicitudes: { solicitudes: 10, editadas: 5 } })
    expect(calcSolicitudes(r)).toBe(5)
  })
})
...
describe('calcTotal + sumScore', () => {
  ...
  it('sumScore con todos los indicadores al máximo es 100', () => {
    const r = makeReport({
      ...
      crecimiento: {
        items: [{ clienteId: 'c1', seguidoresGanados: 100, seguidoresActuales: 1100, meta: 100 }],
      },
    })
