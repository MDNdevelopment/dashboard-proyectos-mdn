...
// ── Mock supabase ────────────────────────────────────────────────────────────
// Las entradas se evalúan de forma perezosa en cada llamada a `.from()` para
// poder leer los overrides de `globalThis.__MOCK_*__` seteados por cada test.
vi.mock('../supabase', () => ({
  supabase: createSupabaseMock({
    tables: {
      ...
      metric_reports: () => makeQuery(globalThis.__MOCK_METRIC_REPORTS__ ?? []),
      meetings: () => makeQuery(globalThis.__MOCK_MEETINGS_OVERRIDE__ ?? []),
    },
  }),
}))
...
describe('HomePage — Mis tareas (nivel 1)', () => {
  ...
  it('la card "Asignadas activas" es un enlace clickeable a Base filtrado por responsable', async () => {
    renderPage({ access_level: 1 })
    await waitFor(() => {
      expect(screen.getByRole('link', { name: /asignadas activas/i })).toHaveAttribute(
        'href',
        '/tareas?view=base&assignee=u1',
      )
    })
  })
  ...
  it('la card "Atrasadas" es un enlace clickeable a Base filtrado por responsable + atrasadas', async () => {
    renderPage({ access_level: 1 })
    await waitFor(() => {
      expect(screen.getByRole('link', { name: /^atrasadas/i })).toHaveAttribute(
        'href',
...
describe('HomePage — nivel 4 (dirección)', () => {
  ...
  it('la card "Apoyo de dirección" es un enlace clickeable a Base filtrado por apoyo, con completadas ocultas por defecto', async () => {
    renderPage({ access_level: 4 })
    await waitFor(() => {
      expect(screen.getByRole('link', { name: /apoyo de dirección/i })).toHaveAttribute(
        'href',
        '/tareas?view=base&support=u1&hideDone=1',
      )
    })
  })
  ...
  it('Clientes, Empleados y Líneas son cards separadas, cada una con su propio enlace', async () => {
    renderPage({ access_level: 4 })
    await waitFor(() => {
      expect(screen.getByRole('link', { name: /clientes activos/i })).toHaveAttribute(
        'href',
        '/empresa/clientes',
      )
    })
...
describe('HomePage — Mi línea (nivel 3)', () => {
  ...
  it('muestra las tareas y la salud de la línea que lidera, y conserva sus cards personales', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole('heading', { name: /^mi línea$/i })).toBeInTheDocument(),
    )
  ...
  it('la card de salud de la línea enlaza a /reportes/linea/:id cuando el usuario tiene acceso a reportes', async () => {
    ...
    renderPage({ access_level: 3 })
    await waitFor(() => {
      expect(screen.getByRole('link', { name: /salud de mi línea/i })).toHaveAttribute(
        'href',
        '/reportes/linea/l1',
      )
    })
  ...
  it('las cards de Clientes y Empleados de la línea enlazan a Empresa con la línea preseleccionada', async () => {
    ...
    renderPage({ access_level: 3 })
    await waitFor(() => {
      expect(screen.getByRole('link', { name: /clientes de mi línea/i })).toHaveAttribute(
        'href',
        '/empresa/clientes?line=l1',
      )
    })
    expect(screen.getByRole('link', { name: /empleados de mi línea/i })).toHaveAttribute(
      'href',
      '/empresa/lineas?line=l1',
    )
  })
...
describe('HomePage — Mis reuniones', () => {
  ...
  it('muestra el texto de estado vacío cuando no hay reuniones esta semana', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/no tienes reuniones agendadas para esta semana/i),
      ).toBeInTheDocument()
    })
  })
