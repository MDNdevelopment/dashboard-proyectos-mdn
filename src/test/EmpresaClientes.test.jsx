...
const MOCK_EMPLOYEES = [
  {
    ...
    department_id: 1,
    avatar_url: null,
    position: { position_name: 'Social Media Manager' },
  },
  {
    user_id: 'emp-2',
    company_id: 'co-1',
  ...
  },
  {
    ...
    department_id: 2,
    avatar_url: null,
    position: { position_name: 'Editor de Video' },
  },
]
...
describe('ClientsView (sección Clientes en Empresa) — lista plana', () => {
  ...
  it('llama a seedMetricsIfEmpty al montar', async () => {
    renderAsManager()
    await waitFor(() => {
      expect(mockSeedIfEmpty).toHaveBeenCalledWith('co-1')
    })
  })
  ...
  it('elegir "No, sacarlo también de este mes" llama a deleteClient con incluyeMes:false', async () => {
    const user = userEvent.setup()
    renderAsManager()
    await waitFor(() => {
      expect(screen.getByText('ALSA')).toBeInTheDocument()
