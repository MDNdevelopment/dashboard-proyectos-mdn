...
describe('aggregateMetricsDashboard', () => {
  ...
  it('identifica el líder del mes de referencia (no el promedio anual)', () => {
    const reports = makeReports(year)
    // En el mes 1: l1 (100) tiene mayor score que l2 (80)
    ...
    expect(agg.lider.score).toBe(agg.matrix['l1'][0])
  })

  it('una línea con un solo mes muy alto no es líder si otra tiene mayor score en el mes de referencia', () => {
    // l1 solo reportó el mes 5 (muy alto); l2 reportó el mes 1 (más bajo).
    // Antes (promedio anual) l1 ganaba por tener un único mes altísimo;
    // ahora el líder se decide por el mes de referencia, así que sin dato
    // de l1 en ese mes, gana l2.
    const reports = [
      { line_id: 'l1', year, month: 5, data: makeReportData(100) },
      { line_id: 'l2', year, month: 1, data: makeReportData(80) },
    ]
    const agg = aggregateMetricsDashboard(
      LINES,
    ...
    expect(agg.lider).not.toBeNull()
    expect(agg.lider.line.id).toBe('l2')
  })
  ...
  it('finanzas por línea muestra solo el mes seleccionado', () => {
    ...
    const agg1 = aggregateMetricsDashboard(
      LINES,
      reports,
      year,
      calcTotal,
    ...
    const agg2 = aggregateMetricsDashboard(
      LINES,
      reports,
      year,
      calcTotal,
    ...
    const agg3 = aggregateMetricsDashboard(
      LINES,
      reports,
      year,
      calcTotal,
  ...
  it('meses marcados como incompletos quedan excluidos de matrix/promAnual/lider', () => {
    ...
    // El lider del mes 1 (referencia) no puede ser determinado por el mes 2
    // incompleto de l1 — se decide por el score del mes 1, donde l1 (100) > l2 (80)
    const aggM1 = aggregateMetricsDashboard(
      LINES,
      reports,
      year,
      calcTotal,
    ...
    expect(aggM1.lider).not.toBeNull()
    expect(aggM1.lider.line.id).toBe('l1')
    ...
    const aggM2 = aggregateMetricsDashboard(
      LINES,
      reports,
      year,
      calcTotal,
