...
| **Tablas** | `auth.users` (Supabase Auth) · `users` (perfil: `user_id, first_name, last_name, email, department_id→departments, position_id→positions, company_id, access_level, admin, avatar_url, receive_ticket_notifications, monthly_salary numeric, deleted_at timestamptz`) · `module_permissions` (reglas de acceso por módulo) |
...
| **Estados** | Ambas tabs comparten los mismos 4 estados, definidos en `constants.js` (`STATUS`/`STATUSES`): `Pendiente`, `En Curso`, `Finalizado`, `Descartado`. Sin CHECK constraint en `campaigns.status`/`paid_campaigns.status` (columna `text` libre) — el set vigente vive solo en `constants.js`. |
