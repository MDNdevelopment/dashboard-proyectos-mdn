...
| **Resultados al finalizar (tab Ads)** | Solo `paid_campaigns` (no aplica a Tácticas). Al marcar un Ad como `Finalizado` es obligatorio capturar 4 resultados — Alcance, Interacciones, Seguidores, Impresiones (`RESULT_FIELDS` en `constants.js`, columnas `reach/interactions/followers/impressions`) — nunca se muestran como columnas de la tabla en pantalla — solo en `AdsSpendDetail.jsx` (sección "Resultados", visible cuando `status === 'Finalizado'`) y, agregadas al final, en el export a Excel (`exportAdsToExcel.js`), vacías si el Ad no está `Finalizado`. La guardia vive centralizada en `AdsSpendView.requestStatusChange`: si el nuevo estado es `Finalizado`, en vez de persistir directo abre `AdsResultsModal.jsx`, que pide los 4 valores (todos requeridos) y solo entonces llama `updateAd(id, { status: 'Finalizado', reach, interactions, followers, impressions })`; cualquier otro estado se persiste directo. Cubre las tres vías de cambio de estado: pill de la fila, pill del header de `AdsSpendDetail` (delegado al padre vía prop `onStatusChange`) y el `<select>` de Estado en `AdsSpendForm.jsx` (revela los 4 inputs inline cuando se elige Finalizado). |
...
| `paid_campaigns` | `20260714000001` (creada como `ads`) + `20260715000000` (renombrada) + `20260716000002` (`responsable_id`) + `20260719000000` (`reach/interactions/followers/impressions`) | Pauta pagada (distinta de `campaigns`, que son tácticas orgánicas). FK `client_id→metric_clients` (`ON DELETE SET NULL`); `client` guarda el nombre denormalizado como snapshot. `responsable_id` texto sin FK (igual que `campaigns.assignee`), resuelto vía `loadAdsResponsables()`. `reach/interactions/followers/impressions` (`integer` nullable, sin CHECK): resultados del Ad, obligatorios al marcar `status = 'Finalizado'` (ver §5 "Resultados al finalizar"). RLS: SELECT abierto a autenticados, INSERT/UPDATE/DELETE gated por `user_can('ads.manage')`. Habilitada en `supabase_realtime` desde la migración de renombrado. |
