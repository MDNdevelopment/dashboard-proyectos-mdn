...
| `metric_clients` | `20260625000000` (mod. hasta `20260710000000`) | Cartera de clientes. FK `line_id→metric_lines`. Columnas añadidas: `contacts jsonb`, `anniversary_date date`, `mdn_since date`, `monthly_fee numeric`, `social_manager_id text` (user_id del Social de la línea), `designer_id text` (user_id del Diseñador de la línea), `audiovisual_ids jsonb` (string[] de user_ids dept Audiovisual de la línea), `apoyo_ids jsonb` (string[] de user_ids de cualquier empleado de la empresa). Sin FK hacia `users` (consistente con `member_user_ids`). |
