...
| **Reportes → Tareas (deep-link)** | Nombres de cliente en `OperacionesView` (Crecimiento, Pautas, Feedback) y en `FinanzasView` (filas de ingresos ligadas a cliente) son botones que navegan a `/tareas?view=base&team={lineId}&client={clienteId}`. `TareasPage` lee `?team=` para pre-seleccionar el team; `BaseView` lee `?client=` (UUID de `metric_clients`) para filtrar tareas por `client_id`, mostrando una pill removible con el nombre del cliente. |
| **Reportes → Evaluaciones (deep-link)** | `LineHubView` (pestaña Resumen de cada línea) carga los miembros de la línea vía `loadCompanyUsers` + `line.member_user_ids` y muestra una tira "Equipo de la línea" cuyos avatares son botones que navegan a `/evaluaciones/empleado/{userId}`. Solo visible cuando `can('evaluaciones')` es true. |
