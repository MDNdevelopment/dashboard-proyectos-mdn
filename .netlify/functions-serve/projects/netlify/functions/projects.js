...
function requireBearer(event) {
  ...
  const a = (0, import_crypto.createHash)("sha256").update(token).digest();
  const b = (0, import_crypto.createHash)("sha256").update(expected).digest();
  if (!(0, import_crypto.timingSafeEqual)(a, b)) {
    return { statusCode: 401, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ error: "Unauthorized" }) };
  }
  return null;
}
...
var normalize = (row) => ({ ...row, createdAt: row.created_at });
var handler = async (event) => {
  ...
  if (isDetail) {
    const { data: data2, error: error2 } = await supabase.from("projects").select("*").eq("id", lastSegment).single();
    if (error2 || !data2) return json(404, { error: "Not found" });
    return json(200, normalize(data2));
  }
